---
title: "ATP DATASET VISUALISATION"
...
This project seeks to analyze data from men’s ATP tennis competitions from the year 1990 up to 2019, including Grand Slams, Masters Series, Tour Level Finals competitions final matches.

The objective of this analysis is to come up with insights regarding the tournament’s final including correlation between variables (e.g. aces and players’ height) as well as the evolution of the players over time, looking at the different variables and using different types of visualizations as tools to facilitate interpretation of the data and communication of the results.

## Dataset Details

The dataset contains information on every tournament final’s single match played since 1990. It includes details on every match’s date, location, tournament type, surface, winner and loser, games/sets played, duration and additional statistics such as players’ rankings, players’ age and height, aces, double faults, break points faced and saved, service points among other helpful stats for both players of the match.
...
### First steps - Modification and cleaning of the Dataset:

ATP dataset was downloaded from Kaggle. First step post downloading the dataset from Kaggle as the file was huge, to remove the unwanted columns and rows and then filtering out the data we were concerned with, we only considered the Tournaments Grand Slam, ATP Masters, and ATP TOUR Finals and that too only the final match of the tournament for the period from 1990 to 2019. Missed few of the giants of the game such as Rod laver and others.

File can be be downloaded from https://www.kaggle.com/sijovm/atpdata
...
The graph was plotted to display number of distinct winners in the above mentioned tournaments were more from Spain whereas the other countries such as Serbia and Switzerland have less, this is one of the reason of the huge success of Spain in group tournaments.
...
The Box Plot was plotted between Type of Surface against the number of aces hit by the winners in the finals. From the plot we can determine that Clay is the hardest surface to hit aces on whereas the Grass court being the best and the Hard Courts. Reason being the ball holds on to clay then grass and clay is slower than grass.
...
The graph was plotted to show the serving stats of the winners of the tournaments. Graph was plotted for the winners for the Double Faults they served, Number of aces, Number of BP Faced and Number of BP Saved.

1)From graph one we can see that Pete Sampras has most number of double faults in finals.
2)From graph 2 and graph 3 we can see that Rafael Nadal has faced and saved more number of breakpoints in finals.
3)From graph 4 we can see Roger Federer & Pete Sampras with most number of aces
...
The graph was plotted to show the number of aces hit by the Left Handed Players and Right Handed Players on different kind of surfaces. From graph we can infer that more number of aces are hit on the hard court in comparison to Grass Court and Clay court and the number of aces hit by Right Handed players are more in comparison to Left Handed Players.
...
Graph was plotted between the number of tournaments played based on the surface, based on the graph we can infer that the most number of tournaments were played on the Hard Courts than on the Grass Court and Clay Court. Grass Court being the least.
...
Scatter Plot was plotted to understand the relationship between the winners height and the total number of aces hit by the players. On reading the graph we can infer that fairly large number of aces are hit by taller players than the shorter or avg height players
...
Dataset was grouped on the tournament winners and summarised for their total tournament wins and then the graph was plotted between the two variables. From the graph we can infer that Roger Federer has been the dominant player for the period followed by Novak and Rafael.
...
The graph was plotted for the countrywise ranking of the players at the tournament wins by their players.In this graph we tried to summarize the distribution of winner ranking by their country. In this boxplot we can see that the mean and median ranking of players from Swiss and serbia are low and that of Britain and other countries are higher

### Winners Break point saved with age distribution as per surface
...
Graph was plotted to study the break points saved by the players against the age of the winners. From the graph we can infer that the younger players are hard to break and they have save more number of break points. Also the breakpoint conversion is very low on Hard courts
...
Histogram was plotted to study the distribution of winner's age of top 5 most dominant countries on tour and for the whole dataset was plotted to understand range and variability. From the graph we can infer that most of the winners range in between 22 years to 30 years of age and there are only few of the winners who are dominant in their later stages of the career.

### Distribution of winners age countrywise

```{r, message=FALSE, warning=FALSE}
plot_ly(data = wins15country, y = wins15country$winner_age,x = wins15country$winner_ioc, type = "box", color = wins15country$winner_ioc)%>% layout(title= "Distribution of age of the winners of the Countries with most title during the period", xaxis = list(title = 'Countries'), yaxis = list(title = 'Ages'))



```

The Box plot was plotted to summarise the age of the winners as per country. On carefully analysing the graph we can infer that median age of the Switzerland and Great Britain is more where as Australian is the least, maximum age of the Swiss ranges beyond 35 showing the dominance of Roger even at the age of 35.
...
#minutes.data1
p1 <- plot_ly(minutes.data1, x= ~minutes.data1$xbar31, type = 'histogram',histnorm = 'probability')%>%
  layout(xaxis = list(title = 'xbar'), yaxis = list(title = 'density'))
...
p2 <- plot_ly(minutes.data2, x= ~minutes.data2$xbar32, type = 'histogram',histnorm = 'probability')%>%
  layout(xaxis = list(title = 'xbar'), yaxis = list(title = 'density'))
...
p3 <- plot_ly(minutes.data3, x= ~minutes.data3$xbar33, type = 'histogram',histnorm = 'probability')%>%
  layout(xaxis = list(title = 'xbar'), yaxis = list(title = 'density'))
...
p4 <- plot_ly(minutes.data4, x= ~minutes.data4$xbar34, type = 'histogram',histnorm = 'probability')%>%
  layout(xaxis = list(title = 'xbar'), yaxis = list(title = 'density'))
...
The Central Limit Theorem states that the distribution of the sample means for a given sample size of the
population has the shape of the normal distribution.So we tried to apply the Central Limit Theorem on the given dataset. The sample means for different sample size between 10 t0 40 with difference were plotted and it was found that the distribution of sample means follow the curve of normal distribution and bell shaped curve is observed.
...
Sampling is used to identify and analyze any trends or patterns that can be seen in a subset of a larger group of data. It can also be useful technique to help predict some type of data or information. There are many different types of sampling that can be applied to data. The sampling methods used for this analysis are simple random sampling with replacement, systematic, and stratified

Simple Random Sampling for the dataset's two numerical variable match Duration and the Number of aces hit by the winners was applied to find out whether the dataset follows simple random Sampling or not. On the basis of the graph and mean comparison of the Sample data and Population data we figured out that the simple random sampling holds good for the data as the mean of the sample and population were find in close proximity.
...
Systematic Sampling for the dataset's two numerical variable match Duration and the Number of aces hit by the winners was applied to find out whether the dataset follows systematic Sampling or not. On the basis of the graph and mean comparison of the Sample data and Population data we figured out that the systematic sampling holds good for the data as the mean of the sample and population were find in close proximity.But the simple random sampling was better in comparison.
...
Stratified Sampling was applied on the dataset, But based on the resultant mean of the sample and comparison with the population, we can infer that Stratified sampling doesn't holds good for the datset.
...
Tennis is one of the most challenging and physically demanding game, still there have been players such as Roger Federer, Novak and Nadal who have dominated the game in the last two decades. The serving stats of the most dominant tennis players are staggering, with less number of Double Faults and saving more than 70% of the breakpoints in the matches and hitting high number of aces. Distribution of the winners age depicts the tennis tournament winners age ranges between 22 to 29 and with height it is easier to hit the aces and Grass court is easier to hit the number of aces in comparison to clay courts

## Conclusion

Atp Dataset for the duration of 1990 to 2019, clearly established Roger Federer the best player to play during the duration and Winners height is positively correlated with the number of aces and it is simpler to hit the aces on Grass and Hard court than clay court and most number of matches are being played on the Hard court. Also we can conclude that success at tournaments declines post thirty years of age and mostly winners range in between 22 to 29 years of age.
